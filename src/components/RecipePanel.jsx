...
const RecipePanel = ({
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-amber-100 px-3 py-2 mx-2 sm:mx-4 space-y-1.5 sm:space-y-2 w-full">
      {/* Recipe name */}
      <div className="font-black text-xs sm:text-base text-amber-900 text-center leading-tight">
        📋 {recipe.name}
      </div>
        ...
        {recipe.ingredients.map((ingredient, idx) => {
          ...
          return (
            <span key={idx} className="flex items-center">
              <span className={`
                inline-flex flex-col items-center justify-center rounded-xl px-1 py-0.5
                text-base sm:text-2xl transition-all duration-200
              ...
              `}>
                <span className="text-[8px] font-black text-amber-600 leading-none">{idx + 1}</span>
                <span>{isDone ? '✅' : ingredient}</span>
              </span>
              {idx < recipe.ingredients.length - 1 && (
                <span className="text-amber-300 mx-0.5 text-xs font-black">→</span>
              )}
            </span>
          );
        })}
      ...
      <div className="flex items-center justify-center gap-0.5">
        {Array.from({ length: maxErrors }, (_, i) => (
          <span key={i} className={`text-xs sm:text-sm ${i < errorsInCurrentDish ? '' : 'opacity-20'}`}>
